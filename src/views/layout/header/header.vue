...
<script>
  ...
  export default {
    ...
    data () {
      return {
        isfullScreen: true,
        nickname:sessionStorage.nickname.replace("\"","").replace("\"",""),
        avatar: sessionStorage.usrimg.replace("\"","").replace("\"",""),
        shijian: ''
      }
    },
    mounted () {
      ...
      _this.shijian = formatDate(new Date(),'yyyy-MM-dd hh:mm:ss');//修改数据date
      }, 
      1000);
    },
    beforeDestroy () {
